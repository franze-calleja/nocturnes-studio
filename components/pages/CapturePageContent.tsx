...
import { frames } from "@/app/data/frames"; // Note: path updated to reflect new location
...
export default function CapturePageContent() {
  ...
  const takePhoto = () => {
    ...
    // Flip horizontally when taking the photo
  ...
  // 6) once we're done, move on
  const handleComplete = () => {
    if (!frameId || capturedPhotos.length !== totalSlots) return;
    // Store only photos in localStorage
    try {
      localStorage.setItem(
        "nocturne-photos",
        JSON.stringify({ photos: capturedPhotos })
      );
